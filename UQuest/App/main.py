# Убираем или комментируем строку с путем к venv, она не является исполняемым кодом Python
# """.venv\Scripts\activate                 uvicorn App.main:app --reload            docker-compose up --build --force-recreate"""

from fastapi import FastAPI, HTTPException, Response
from pydantic import BaseModel  # pydantic используеться для валидации ( Цель валидации — убедиться, что данные корректны) данных, работа с pydantic обзывается как схема (Schema)
...
from database.models import Base, UserModel, HistoryModel 
from AUF.auf import security, config 
from fastapi.middleware.cors import CORSMiddleware
from datetime import datetime, date # date уже импортирован
from fastapi import Query, Request # Добавляем Request
from fastapi.concurrency import run_in_threadpool # Импортируем для асинхронного выполнения синхронных функций
from typing import List

# Модель для тела запроса /math
class MathBody(BaseModel):
    vremya: int # Фактически затраченное время в секундах
    chislo: int # Оценка ИИ в секундах

# Модель для тела запроса нового эндпоинта подсчета сэкономленного времени
class CompleteTaskBody(BaseModel):
    task: str  # Описание задачи
    actual_time_seconds: int # Фактически затраченное пользователем время в секундах
...
async def logout(response: Response):
    """
    Выход пользователя путем удаления cookie с токеном доступа.
    """
